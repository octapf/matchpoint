  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (tournamentId && typeof tournamentId === 'string') {
        const tournamentIdTrimmed = tournamentId.trim();
        const readable = await assertTournamentReadable(req, db, tournamentIdTrimmed);
        if (!readable.ok) return corsRes.status(readable.status).json({ error: readable.error });
        Object.assign(filter, tournamentIdMongoFilter(tournamentIdTrimmed));
    ...
    if (req.method === 'POST') {
      ...
      if (totalTeams >= maxT) {
        return corsRes.status(400).json({ error: 'Tournament is full — join the team slot waitlist' });
      }
