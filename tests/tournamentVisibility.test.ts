...
function fakeDb(
  ...
  opts: { entryUserId?: string; waitlistUserId?: string; adminUserId?: string } = {}
): Db {
  return {
    collection(name: string) {
      return {
        async findOne(query: Record<string, unknown>) {
          ...
          if (name === 'users') {
            const id = query._id instanceof ObjectId ? query._id.toString() : '';
            if (id === opts.adminUserId) return { _id: query._id, role: 'admin' };
            return id ? { _id: query._id } : null;
          }
          if (name === 'entries') {
...
describe('assertTournamentReadable', () => {
  ...
  it('treats private visibility case-insensitively', async () => {
    const db = fakeDb({ _id: new ObjectId(tournamentId), visibility: 'Private' });
  ...
  it('allows private tournaments for admins', async () => {
    const db = fakeDb({ _id: new ObjectId(tournamentId), visibility: 'private' }, { adminUserId: adminId });

    const result = await assertTournamentReadable(reqFor(adminId), db, tournamentId);
